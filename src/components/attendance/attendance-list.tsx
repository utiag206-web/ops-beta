...
export function AttendanceList({ records, isWorker = false }: AttendanceListProps) {
 ...
 <span className="text-[10px] font-black px-2.5 py-1 rounded-lg border bg-emerald-50 text-emerald-700 border-emerald-100 shadow-sm">
 Puntual
 </span>
 )
 ) : (
 ...
 <span className="text-[9px] font-bold text-slate-500 tracking-tighter">{isWorker ? 'Mi Marcación' : record.worker?.name || 'Sistema'}</span>
 </div>
 </div>
 ))}
 </div>
