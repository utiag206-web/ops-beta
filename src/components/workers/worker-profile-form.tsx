...
const Input = ({ label, value, onChange, type = "text", required = false, disabled = false, autoComplete = "off" }: any) => (
 ...
 <label className="text-xs font-bold text-slate-500">{label} {required && <span className="text-rose-500">*</span>}</label>
 <input 
 type={type} 
 value={value || ''} 
 onChange={onChange}
...
export function WorkerProfileForm({ worker, childrenList = [], canManage = false }: { worker: any, childrenList?: any[], canManage?: boolean }) {
 ...
 status: worker.current_status || worker.status || 'active',
 ...
 value={laboral.status} 
 onChange={(e:any) => setLaboral(prev => ({...prev, status: e.target.value}))}
 disabled={!canManage}
 options={[
 ...
 />
 <div className="space-y-1.5 flex-1 min-w-[200px]">
 <label className="text-xs font-bold text-slate-500">Nº Hijos Registrados</label>
