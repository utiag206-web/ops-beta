  ...
  return (
    <div className="space-y-3.5 sm:space-y-4">
        ...
        {canManage && (
          <div className="flex items-center gap-2">
            <button 
              ...
              className="flex-1 md:flex-none flex items-center justify-center gap-1.5 bg-white border border-slate-200 text-slate-700 px-3.5 py-2 rounded-xl text-xs font-bold hover:bg-slate-50 transition-all shadow-xs active:scale-95 disabled:opacity-50"
            >
            ...
            <Link 
              href="/workers/import"
              className="flex-1 md:flex-none flex items-center justify-center gap-1.5 bg-white border border-slate-200 text-slate-700 px-3.5 py-2 rounded-xl text-xs font-bold hover:bg-slate-50 transition-all shadow-xs active:scale-95"
            >
              <Upload size={15} />
              <span>Importar</span>
            </Link>
            <button 
              ...
              className="flex-1 md:flex-none flex items-center justify-center gap-1.5 bg-blue-600 text-white px-4 py-2 rounded-xl text-xs font-bold hover:bg-blue-700 transition-all shadow-sm shadow-blue-200 active:scale-95"
            >
              <Plus size={15} />
              <span>Nuevo</span>
            </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="md:col-span-2 relative">
          ...
          <input 
            ...
            className="w-full bg-white border border-slate-200 focus:border-blue-500 rounded-xl py-2 pl-9 pr-3 text-xs font-medium transition-all outline-none shadow-xs"
          />
        </div>
        <div className="flex gap-2">
          <button className="flex-1 bg-white border border-slate-200 hover:bg-slate-50 rounded-xl text-xs font-bold text-slate-700 flex items-center justify-center gap-1.5 transition-all shadow-xs py-2">
            <Filter size={15} />
            Filtrar
          </button>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-xs border border-slate-100 overflow-hidden">
        {/* Pestañas de Estado */}
        <div className="flex border-b border-slate-100 bg-slate-50/30 px-4 pt-3">
          <button
            onClick={() => setActiveTab('activo')}
            className={`pb-2.5 px-4 text-xs font-bold tracking-tight border-b-2 transition-all cursor-pointer ${
              activeTab === 'activo' 
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    <th className="py-2.5 sm:py-3 px-4 sm:px-5 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Colaborador</th>
                    <th className="py-2.5 sm:py-3 px-4 sm:px-5 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Documento</th>
                    <th className="py-2.5 sm:py-3 px-4 sm:px-5 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Puesto / Cargo</th>
                    <th className="py-2.5 sm:py-3 px-4 sm:px-5 text-[10px] font-bold text-slate-400 uppercase tracking-wider text-center">Estado</th>
                    <th className="py-2.5 sm:py-3 px-4 sm:px-5 text-[10px] font-bold text-slate-400 uppercase tracking-wider text-right">Acciones</th>
                  </tr>
                </thead>
                    ...
                    <tr key={worker.id} className="hover:bg-slate-50/50 transition-colors group">
                      <td className="py-2.5 sm:py-3 px-4 sm:px-5">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-xl bg-slate-100 flex items-center justify-center overflow-hidden border border-slate-200 shadow-xs transition-transform group-hover:scale-105 shrink-0">
                            {worker.photo_url ? (
                          ...
                          <div>
                            <p className="text-xs sm:text-sm font-bold text-slate-800 tracking-tight">{worker.name}</p>
                            <p className="text-[10px] text-slate-400 mt-0.5">{worker.hire_date ? `Ingreso: ${new Date(worker.hire_date).toLocaleDateString()}` : 'Sin fecha de ingreso'}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-2.5 sm:py-3 px-4 sm:px-5">
                        <div className="flex flex-col">
                          <span className="text-[9px] font-bold text-blue-600 tracking-tight">DNI / CE</span>
                          <span className="text-xs font-bold text-slate-700">{worker.dni}</span>
                        </div>
                      </td>
                      <td className="py-2.5 sm:py-3 px-4 sm:px-5">
                        <div className="flex flex-col">
                          <span className="text-xs font-bold text-slate-700 capitalize">{worker.position}</span>
                        </div>
                      </td>
                       <td className="py-2.5 sm:py-3 px-4 sm:px-5 text-center">
                         <div className="flex flex-col items-center gap-1">
                           <div className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full border shadow-xs ${
