...
export function TransportList({ payments, isWorker = false }: TransportListProps) {
 ...
 <p className="text-sm font-bold text-slate-800">Pasajes / Movilidad</p>
 </div>
 </div>
 ...
 <div>
 <p className="text-[10px] font-bold text-slate-400 tracking-tight mb-1">Reembolso</p>
 ...
 <span className="text-[10px] font-bold text-slate-500">{isWorker ? 'Mi Registro' : payment.worker?.name || 'Sistema'}</span>
 </div>
 </div>
 ))}
 </div>
