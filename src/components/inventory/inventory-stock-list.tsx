...
function StockGroup({ 
 ...
 <span className="text-[10px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-100 tracking-tight">
 ...
 <p className="text-[9px] font-bold text-slate-400 tracking-[0.2em] mb-1">Disponibilidad Total</p>
 ...
 {group.totalQuantity} <span className="text-[10px] font-bold opacity-60">{(Array.isArray(group.product) ? group.product[0]?.unit : group.product?.unit) || 'UND'}</span>
 </div>
 </div>
 ...
 <div>
 <p className="text-sm font-bold text-slate-700">{item.warehouses?.name || item.location}</p>
...
function TraceabilityCard({ productId, unit }: { productId: string, unit: string }) {
 ...
 <p className="text-xs font-bold tracking-tight">Cargando historial...</p>
 </div>
 ) : movements.length > 0 ? (
 <>
 ...
 <div>
 <p className="text-[10px] font-bold text-slate-700 leading-none mb-1 group-hover:text-indigo-600 transition-colors">
 ...
 <p className="text-[9px] font-bold text-slate-400">{new Date(m.created_at).toLocaleDateString()} {new Date(m.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
 </div>
 </div>
 ...
 <p className="text-[9px] font-bold text-slate-400 tracking-tighter">{m.warehouses?.name || m.location}</p>
 </div>
 </div>
 ))}
 </div>
...
function MovementDetailModal({ 
 ...
 <div>
 <p className="text-[10px] font-bold tracking-tight text-slate-500">Cantidad Registrada</p>
 <p className="text-3xl font-extrabold tracking-tight mt-1">
 {isSalida ? '-' : '+'}{movement.quantity}
 <span className="text-sm font-bold opacity-75 ml-2">{unit || 'UND'}</span>
 </p>
 </div>
 ...
 <p className="text-[10px] font-bold text-slate-400 tracking-normal mb-1">Descripción del Producto</p>
 ...
 <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
 <p className="text-[10px] font-bold text-slate-400 tracking-normal mb-1">Concepto / Tipo</p>
 <p className="text-sm font-bold text-slate-700">
 {movement.movement_types?.name || movement.type || 'N/A'}
 </p>
 </div>
 <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
 <p className="text-[10px] font-bold text-slate-400 tracking-normal mb-1">Almacén / Ubicación</p>
 ...
 <div className="grid grid-cols-2 gap-4">
 <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
 <p className="text-[10px] font-bold text-slate-400 tracking-normal mb-1">Tipo de Documento</p>
 <p className="text-sm font-bold text-slate-700">
 {movement.document_type || 'SIN DOCUMENTO'}
 </p>
 </div>
 <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
 <p className="text-[10px] font-bold text-slate-400 tracking-normal mb-1">Número de Documento</p>
 <p className="text-sm font-bold text-slate-700">
 ...
 <span className="text-[10px] font-bold text-slate-400 tracking-normal">Fecha y Hora</span>
 <span className="text-xs font-bold text-slate-600">
 {new Date(movement.created_at).toLocaleDateString()} {new Date(movement.created_at).toLocaleTimeString()}
 </span>
 </div>
 <div className="flex justify-between items-center pb-2 border-b border-slate-200/50">
 <span className="text-[10px] font-bold text-slate-400 tracking-normal">Registrado Por</span>
 ...
 <span className="text-[10px] font-bold text-slate-400 tracking-normal">Referencia</span>
 ...
 {/* Observations */}
 <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
 <p className="text-[10px] font-bold text-slate-400 tracking-normal mb-2 flex items-center gap-1.5">
