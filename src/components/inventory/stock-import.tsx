          ...
          <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-200 rounded-xl p-12 bg-slate-50">
            ...
            <div className="flex flex-wrap gap-4 justify-center mt-4">
              <button 
                ...
                className="bg-white border-2 border-emerald-200 hover:bg-emerald-50 text-emerald-600 px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 shadow-sm"
              >
                <Download size={16} className="text-emerald-600" /> Descargar Plantilla
              </button>
              <label className="bg-emerald-600 hover:bg-emerald-700 !text-white px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest cursor-pointer transition-all shadow-md flex items-center gap-2">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-6 gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-bold uppercase tracking-widest">{data.length} registros cargados</p>
                </div>
              </div>
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <button 
                  ...
                  className="w-full sm:w-auto flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 disabled:text-slate-400 text-white px-8 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-emerald-100 active:scale-95 disabled:shadow-none"
                >
              ...
              <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead>
                  <tr className="bg-slate-50 sticky top-0 z-10 border-b border-slate-150">
                    <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Código Producto (SKU)</th>
                    <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Producto</th>
                    <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider">Cantidad</th>
                    ...
                    <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider text-center">Estado / Validación</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {data.map((row, i) => (
                    <tr key={i} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm text-slate-600 uppercase">{row.unidad}</td>
                      <td className="px-4 py-3 text-sm text-slate-700 font-bold">{row.almacen}</td>
                      <td className="px-4 py-3 text-center">
                        {row.isValid ? (
                          <span className="px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest bg-green-50 text-green-700 border border-green-100 flex items-center justify-center gap-1 w-max mx-auto">
                            <Check size={10} strokeWidth={3} /> Válido
                          </span>
                        ) : (
                          <span className="px-2.5 py-1 rounded-full text-[9px] font-black uppercase tracking-widest bg-rose-50 text-rose-700 border border-rose-100 flex items-center justify-center gap-1 w-max mx-auto" title={row.errorMsg}>
