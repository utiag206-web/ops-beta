    ...
    <div className="space-y-4">
          ...
          <div className="flex items-center gap-2 bg-slate-50 px-3 py-2 border border-slate-200 rounded-xl">
            ...
            {dateFilter && (
              <button 
                ...
                className="text-xs text-rose-500 hover:text-rose-600 font-bold ml-1 uppercase"
              >
                Limpiar
              </button>
            )}
          ...
          {canWrite && (
            <div className="flex items-center gap-2">
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all shadow-sm active:scale-95 border border-blue-500/10"
              >
                <Plus size={14} />
                Fila Nueva
              </button>
              ...
              {Object.values(selectedIds).some(Boolean) && (
                <button 
                  ...
                  disabled={isPending}
                  className="flex items-center gap-2 px-4 py-2.5 bg-rose-600 hover:bg-rose-505 disabled:opacity-50 text-white font-bold rounded-xl text-xs uppercase tracking-wider transition-all shadow-sm active:scale-95 border border-rose-500/10 animate-pulse"
                >
                  <Trash2 size={14} />
                  Eliminar
                </button>
      ...
      {errorMessage && (
        <div className="flex items-center gap-3 bg-rose-50 border-l-4 border-rose-600 text-rose-900 p-4 rounded-xl shadow-sm animate-in slide-in-from-top-4 duration-300">
          ...
          <div className="text-xs font-bold uppercase">{errorMessage}</div>
        </div>
      )}
          ...
          <table className="w-max text-left border-collapse table-fixed select-none">
            <thead>
              <tr className="bg-slate-50 text-slate-700 border-b border-slate-200">
                ...
                {/* Status Column */}
                <th className="sticky left-12 z-30 bg-slate-50 border-r border-slate-200 px-2 py-4 text-center w-12 align-middle text-[10px] font-black uppercase tracking-wider text-slate-500">
                  EST
                </th>
                ...
                {columns.filter(col => !col.hideInGrid).map((col, idx) => {
                  ...
                  return (
                    <th 
                      key={col.key} 
                      className={`
                        border-r border-slate-200 px-4 py-4 text-[10px] font-black uppercase tracking-widest align-middle text-slate-600
                ...
                <tr>
                  <td colSpan={columns.length + 2} className="py-24 text-center text-slate-400 bg-slate-50/50">
                    <div className="flex flex-col items-center justify-center gap-3">
                      ...
                      <span className="font-black text-sm uppercase tracking-wider text-slate-500">Sin registros en esta hoja</span>
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-2xl px-6 py-3.5 flex flex-wrap gap-x-6 gap-y-2 items-center justify-between text-[11px] font-black text-slate-500 uppercase tracking-wider">
        <div className="flex items-center gap-2">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="p-6 border-t border-slate-100 flex items-center justify-end gap-3 bg-slate-50/50">
            <button
              ...
              onClick={onClose}
              disabled={loading}
              className="px-5 py-3 border border-slate-200 text-slate-500 rounded-xl font-bold hover:bg-slate-100 transition-all text-xs uppercase tracking-wider disabled:opacity-50"
            >
              Cancelar
            </button>
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-black transition-all shadow-md shadow-blue-500/10 text-xs uppercase tracking-wider flex items-center gap-2 disabled:opacity-50"
            >
              {loading ? (
                <>
