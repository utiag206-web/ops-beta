...
export function StatWidget({ title, value, icon: Icon, color, bg, href, trend, badge }: any) {
 const content = (
 <div className="bg-white p-5 md:p-6 rounded-2xl md:rounded-3xl shadow-sm border border-slate-100 flex flex-col justify-between gap-4 md:gap-5 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group h-full relative overflow-hidden">
 ...
 {badge && (
 <div className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-[10px] font-bold tracking-wider ${badge.color || 'bg-rose-50 border border-rose-100 text-rose-600'}`}>
 {badge.text}
 </div>
 )}
 {trend && (
 <div className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-[10px] font-bold tracking-wider ${
 ...
 <p className="text-[11px] font-semibold text-slate-500 tracking-[0.18em] mb-2 leading-none">{title}</p>
 <p className={`font-bold text-slate-900 group-hover:text-blue-600 transition-colors leading-none ${
...
export function AlertWidget({ title, message, icon: Icon, color, bg, href }: any) {
 ...
 <div>
 <p className="text-[10px] font-bold text-slate-500 tracking-[0.15em] mb-1">{title}</p>
...
export function ListWidget({ title, items, icon: Icon, color, hrefLabel, href }: any) {
 ...
 {href && (
 <Link href={href} className="text-[10px] font-bold text-blue-600 hover:text-white hover:bg-blue-600 flex items-center gap-2 px-5 py-2.5 bg-blue-50 rounded-xl transition-all tracking-tight shadow-sm">
 ...
 <p className="text-sm font-bold text-slate-800 truncate tracking-tight group-hover/item:text-blue-600 transition-colors">{item.title || item.name}</p>
 <p className="text-[11px] font-bold text-slate-400 truncate mt-1">{item.subtitle}</p>
 </div>
 {item.badge && (
 <span className={`text-[9px] font-bold px-3 py-1.5 rounded-xl ml-4 shadow-sm border ${item.badgeColor || 'bg-white text-slate-600 border-slate-100'}`}>
 {item.badge}
 </span>
 )}
 </div>
 ...
 <p className="text-xs font-bold text-slate-300 tracking-tight leading-loose">No hay actividad reciente</p>
 </div>
 )}
 </div>
 </div>
...
export function WelcomeHero({ userName, roleName, area, companyName, viewMode, companySlug, localIp }: any) {
 ...
 <span 
 style={{ color: '#ffffff' }}
 className="bg-white/10 backdrop-blur-md text-[9px] font-semibold px-4 py-1.5 rounded-lg keep-case tracking-normal border border-white/10"
 >
 ...
 <h1 
 style={{ color: '#ffffff' }}
 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-2 tracking-tight leading-tight"
 >
 {content.title} 👋
 </h1>
 <p 
 ...
 <div>
 <p className="text-[9px] font-bold text-blue-100 tracking-tight leading-none mb-1.5">Acceso Rápido</p>
 <h4 className="text-sm font-bold text-white tracking-normal leading-none">Acceso Trabajadores</h4>
 </div>
 </button>
 </div>
 )}
 ...
 <h3 className="text-xl font-bold tracking-tight leading-none">Acceso de Trabajadores</h3>
 <p className="text-blue-100 text-[10px] font-bold tracking-tight mt-1.5">{companyName}</p>
 </div>
 ...
 className="flex items-center justify-center gap-2 p-3.5 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold rounded-2xl transition-all border border-slate-100 text-[10px] tracking-normal cursor-pointer active:scale-95"
 >
 ...
 } font-bold rounded-2xl transition-all border text-[10px] tracking-wider cursor-pointer active:scale-95`}
 >
 ...
 className="flex items-center justify-center gap-2 p-3.5 bg-blue-50 hover:bg-blue-100 text-blue-600 font-bold rounded-2xl transition-all border border-blue-100/50 text-[10px] tracking-normal cursor-pointer active:scale-95"
 >
 <Share2 size={15} />
 Compartir
 </button>
