...
export function StatWidget({ title, value, icon: Icon, color, bg, href, trend }: any) {
  const content = (
    <div className="bg-white p-5 md:p-7 rounded-2xl md:rounded-[2rem] shadow-sm border border-slate-100 flex flex-col justify-between gap-4 md:gap-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group h-full relative overflow-hidden">
      ...
      <div className="flex items-start justify-between relative z-10">
        ...
        {trend && (
          <div className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase tracking-tighter ${
      ...
      <div className="relative z-10">
        <p className="text-[9px] md:text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-1 opacity-80">{title}</p>
        <p className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tighter group-hover:text-blue-600 transition-colors">{value}</p>
      </div>
    </div>
  )
...
export function WelcomeHero({ userName, roleName, area, companyName, viewMode }: any) {
  ...
  return (
    <div className="bg-[#1D4ED8] rounded-2xl md:rounded-[2rem] p-6 md:p-10 text-white shadow-2xl relative overflow-hidden group">
      {/* Background effects */}
      ...
      <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-10">
        <div className="max-w-2xl">
          <div className="flex flex-wrap items-center gap-3 mb-6">
            <span className="bg-white/10 backdrop-blur-xl text-white text-[10px] font-bold px-5 py-2 rounded-xl uppercase tracking-[0.2em] border border-white/10 shadow-xl">
              {roleName} {area ? `| ${area}` : ''}
            </span>
        ...
        <div className="flex items-center gap-8 shrink-0">
           <div className="text-center p-6 bg-white/10 backdrop-blur-md rounded-[2rem] border border-white/20 hover:bg-white/15 transition-all">
              <p className="text-[10px] font-bold text-white uppercase tracking-widest mb-2">Estado Global</p>
              <div className="flex items-center gap-2">
                <span className="w-2.5 h-2.5 bg-emerald-400 rounded-full animate-pulse shadow-[0_0_15px_rgba(52,211,153,0.6)]" />
                <p className="text-xl font-bold text-white uppercase tracking-tighter">Operativo</p>
              </div>
           </div>
