...
function LoginForm() {
  ...
  let noticeType: 'info' | 'warning' = 'info'
  if (errorParam === 'pending') {
    noticeMessage = 'Su empresa o cuenta se encuentra en proceso de revisión y activación por el Super Administrador.'
    noticeType = 'warning'
  } else if (errorParam === 'inactive') {
    noticeMessage = 'La empresa asociada se encuentra inactiva o suspendida. Comuníquese con soporte.'
    noticeType = 'warning'
  }
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden">
        ...
        <div className="p-8">
          <form action={formAction} onSubmit={handleSubmit} className="space-y-5">
            {noticeMessage && (
              <div className={`p-3 border rounded-lg flex items-center gap-2 text-sm font-medium ${
                noticeType === 'warning' ? 'bg-amber-50 border-amber-200 text-amber-800' : 'bg-blue-50 border-blue-200 text-blue-700'
              }`}>
            ...
            <div className="text-center mt-5 pt-3 border-t border-slate-100">
              <p className="text-xs text-slate-500">
                ¿Tu empresa aún no cuenta con acceso corporativo?{' '}
