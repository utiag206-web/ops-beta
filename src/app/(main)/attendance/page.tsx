...
export default async function AttendancePage(props: { searchParams: Promise<{ date?: string }> }) {
  ...
  const records = await getAttendance(undefined, searchParams.date)
 ...
 <div>
 <h1 className="text-2xl font-black text-slate-800 tracking-tight">Mi Asistencia</h1>
 ...
 <p className="text-[10px] font-black text-slate-400 tracking-tight mb-1">Días Laborados</p>
 <p className="text-2xl font-black text-slate-800">{totalDays}</p>
 <span className="text-[8px] font-bold text-slate-400 tracking-normal">Últimos 30 días</span>
 </div>

 <div className="bg-emerald-50/50 p-5 rounded-2xl border border-emerald-100/50 shadow-sm">
 <p className="text-[10px] font-black text-slate-400 tracking-tight mb-1">Puntuales</p>
 <p className="text-2xl font-black text-emerald-700">{puntuales}</p>
 <span className="text-[8px] font-black text-emerald-600/80 tracking-normal flex items-center gap-1">
 <CheckCircle2 size={10} /> Ingresos a tiempo
 </span>
 </div>

 <div className="bg-amber-50/50 p-5 rounded-2xl border border-amber-100/50 shadow-sm">
 <p className="text-[10px] font-black text-slate-400 tracking-tight mb-1">Tardanzas</p>
 <p className="text-2xl font-black text-amber-700">{tardanzas}</p>
 <span className="text-[8px] font-black text-amber-600/80 tracking-normal flex items-center gap-1">
 <AlertTriangle size={10} /> Pasado las 08:05 AM
 </span>
 </div>

 <div className="bg-blue-50/50 p-5 rounded-2xl border border-blue-100/50 shadow-sm">
 <p className="text-[10px] font-black text-slate-400 tracking-tight mb-1">Incompletas</p>
 <p className="text-2xl font-black text-blue-700">{incompletas}</p>
 <span className="text-[8px] font-black text-blue-600/80 tracking-normal flex items-center gap-1">
 ...
 <h2 className="text-md font-black text-slate-800 tracking-tight mb-4 flex items-center gap-2">
 ...
 <AttendanceList records={records} isWorker={true} />
  ...
  // Administrative view
  const currentRecords = records.filter((r: any) => r.date === currentDate)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-gradient-to-br from-blue-700 to-blue-900 p-8 rounded-3xl text-white shadow-lg">
            <h3 className="text-4xl font-black mb-1">{currentRecords.length}</h3>
            <p className="text-blue-100 text-sm font-medium">
              {currentDate === today ? 'Marcaciones Hoy' : `Marcaciones ${new Date(currentDate + 'T12:00:00').toLocaleDateString()}`}
            </p>
 ...
 <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
              ...
              <AttendanceDateFilter defaultDate={today} />
            </div>
          </div>
