...
export default function CajaChicaPage() {
 ...
 const isAdmin = role_id === 'admin' || role_id === 'super_admin' || role_id === 'superadmin'
 ...
 <h1 className="text-xl font-black text-slate-800 tracking-tight">Caja Chica Deshabilitada</h1>
 <p className="text-xs font-bold text-amber-600 tracking-tight">Área No Asignada</p>
 </div>
 ...
 {/* Header Section */}
 <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
 ...
 <div>
 {['admin', 'gerente', 'super_admin', 'superadmin'].includes(role_id || '') ? (
 <div className="flex flex-col gap-1">
 <p className="text-[10px] font-bold text-blue-600 tracking-tight ml-1">Vista Administrativa Central</p>
 <select 
 value={area}
 ...
 <>
 <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Caja Chica: {area}</h1>
 ...
 <div className="bg-slate-900 p-8 rounded-[2rem] shadow-2xl relative overflow-hidden col-span-1 md:col-span-1">
 <div className="absolute top-4 right-4 text-white/10 rotate-12">
 <Wallet size={80} />
 </div>
 <p className="text-[10px] font-bold text-white/40 tracking-tight mb-2">Saldo en Caja</p>
 ...
 <span className="px-3 py-1 bg-white/10 rounded-full text-[10px] font-bold text-white tracking-tighter">Fondo Activo</span>
 </div>
 </div>
 ...
 <p className="text-[10px] font-bold text-slate-400 tracking-tight mb-1">Ingresos (Mes)</p>
 ...
 <p className="text-[10px] font-bold text-slate-400 tracking-tight mb-1">Gastos (Mes)</p>
 <h3 className="text-2xl font-bold text-slate-900 tracking-tighter">
 ...
 <div className="bg-blue-50 p-6 rounded-[2rem] border border-blue-100 flex flex-col justify-center gap-4 col-span-1">
 <div className="flex justify-between items-center text-[11px] font-bold text-blue-900/60">
 <span>Período</span>
 ...
 <p className="text-[9px] font-bold text-blue-400 text-right">USO DE RECURSOS DEL MES</p>
 </div>
 </div>
 </div>
 ...
 <h3 className="text-2xl font-bold text-slate-800 tracking-tight">Historial de Movimientos</h3>
 <span className="px-4 py-1.5 bg-slate-900 text-white text-[10px] font-bold rounded-full tracking-tight">
 ...
 <th className="py-5 px-8 text-[11px] font-bold text-slate-400 tracking-tight">Fecha</th>
 <th className="py-5 text-[11px] font-bold text-slate-400 tracking-tight">Responsable / Categoría</th>
 <th className="py-5 text-[11px] font-bold text-slate-400 tracking-tight">Motivo / Concepto</th>
 <th className="py-5 text-[11px] font-bold text-slate-400 tracking-tight text-center">Referencia</th>
 <th className="py-5 text-[11px] font-bold text-slate-400 tracking-tight text-right">Monto</th>
 <th className="py-5 px-8 text-[11px] font-bold text-slate-400 tracking-tight text-center">Acciones</th>
 </tr>
 </thead>
 ...
 <span className="text-lg font-bold text-slate-400">Sin movimientos que mostrar</span>
 </div>
 </td>
 </tr>
 ...
 <span className={`text-[9px] font-bold leading-none mb-1 ${t.type === 'ingreso' ? 'text-emerald-400' : 'text-slate-400'}`}>
 {format(new Date(t.date), 'MMM').toUpperCase()}
 </span>
 <span className={`text-base font-bold leading-none ${t.type === 'ingreso' ? 'text-emerald-700' : 'text-slate-700'}`}>
 ...
 <div className="flex items-center gap-2">
 <span className="text-[10px] font-bold text-slate-400">{t.responsible?.name}</span>
 <ChevronRight size={10} className="text-slate-300" />
 </div>
 <div className="flex items-center gap-2">
 <span className="text-xs">{CATEGORY_MAP[t.category]?.icon}</span>
 <span className="text-sm font-bold text-slate-800">{CATEGORY_MAP[t.category]?.label || 'General'}</span>
 </div>
 </div>
 </td>
 ...
 <span className="text-[9px] font-bold text-slate-500 tracking-tighter">{t.payment_method}</span>
 </div>
 </div>
 </td>
 ...
 {t.operation_number && (
 <span className="px-2.5 py-1 bg-blue-50 text-blue-600 text-[9px] font-bold rounded-lg border border-blue-100 flex items-center gap-1">
 ...
 href={t.voucher_url} 
 target="_blank" 
 className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 border border-slate-800 text-white text-[9px] font-bold rounded-xl hover:bg-slate-800 hover:-translate-y-0.5 transition-all shadow-md shadow-slate-200"
 >
 ...
 <span className={`text-[9px] font-bold tracking-widest ${t.type === 'ingreso' ? 'text-emerald-400' : 'text-slate-400'}`}>
