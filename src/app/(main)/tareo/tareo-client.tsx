...
const autoColors: any = {
 ...
 'P': 'bg-yellow-50 text-yellow-600 font-bold',
}
...
export default function TareoClient({ initialCycles, workers, userRole }: TareoPageProps) {
 ...
 const channel = supabase.channel('tareo_attendance_changes')
 ...
 <div>
 <h1 className="text-2xl font-black text-slate-800 tracking-tight">Matriz de Tareo</h1>
 ...
 <div className="px-3 font-black text-slate-700 min-w-[120px] text-center text-sm">
 ...
 <span className="text-[10px] font-black text-slate-500">Horas Diarias</span>
 <input 
 type="number" 
 step="0.01"
 value={dailyHours}
 ...
 <span className="text-[10px] font-black text-slate-400 tracking-tight mr-2 flex items-center">Leyenda:</span>
 {[
  { c: 'P', l: 'Presente', bg: 'bg-yellow-200 text-yellow-900' },
  { c: 'F', l: 'Falta', bg: 'bg-rose-100 text-rose-700' },
  { c: 'AD', l: 'Adelanto', bg: 'bg-slate-200 text-slate-800' },
 ...
 <table className="w-max text-left border-collapse whitespace-nowrap table-fixed">
  <thead>
  <tr className="bg-slate-700/95 backdrop-blur-sm text-white">
  <th rowSpan={2} className="sticky left-0 z-20 bg-slate-700/95 border border-slate-400/50 px-1 md:px-2 py-1 text-[10px] md:text-[11px] font-bold text-center w-[24px] md:w-[40px] align-middle">ITEM</th>
  <th rowSpan={2} className="sticky left-[24px] md:left-[40px] z-20 bg-slate-700/95 border border-slate-400/50 px-1 md:px-3 py-1 text-[10px] md:text-[11px] font-bold text-center w-[90px] md:w-[180px] lg:w-[256px] align-middle">Apellidos y Nombres</th>
  <th rowSpan={2} className="sticky left-[114px] md:left-[220px] lg:left-[296px] z-20 bg-slate-700/95 border border-slate-400/50 px-1 md:px-3 py-1 text-[9px] md:text-[11px] font-bold text-center w-[56px] md:w-[120px] lg:w-[144px] align-middle">Cargo</th>
  {daysInMonth.map((d) => (
  <th key={`wd-${d.day}`} className={`px-1 py-1 text-center border border-slate-400/50 min-w-[28px] md:min-w-[32px] ${d.isWeekend ? 'text-rose-400' : 'text-slate-100'}`}>
  <div className="text-[12px] font-black uppercase tracking-wider">{d.weekday === 'D' ? 'D' : d.weekday.charAt(0).toUpperCase()}</div>
  </th>
  ))}
  <th colSpan={5} className="bg-orange-600/95 border border-slate-400/50 px-2 py-0.5 text-[10px] font-bold text-center text-white">Resumen</th>
  </tr>
  <tr className="bg-slate-700/95 text-white">
  {daysInMonth.map((d) => (
  <th key={`d-${d.day}`} className="px-1 py-1 text-center border border-slate-400/50 min-w-[28px] md:min-w-[32px]">
  <div className="text-[11px] font-black">{String(d.day).padStart(2, '0')}</div>
  </th>
  ))}
  <th className="bg-blue-600/95 border border-slate-400/50 px-1 py-1 text-[10px] font-black text-center text-white w-16">DÍAS TRAB.</th>
  <th className="bg-blue-600/95 border border-slate-400/50 px-1 py-1 text-[10px] font-black text-center text-white w-16">HORAS DIA</th>
  <th className="bg-blue-600/95 border border-slate-400/50 px-1 py-1 text-[10px] font-black text-center text-white w-16">TOTAL HH</th>
  <th className="bg-blue-600/95 border border-slate-400/50 px-1 py-1 text-[10px] font-black text-center text-white w-16">DÍAS LIB.</th>
  <th className="bg-blue-600/95 border border-slate-400/50 px-1 py-1 text-[10px] font-black text-center text-white w-16">TOTAL DÍAS</th>
  </tr>
  </thead>
 <tbody>
...
const WorkerRow = React.memo(function WorkerRow({ worker, idx, daysInMonth, workerStatuses, dailyHours, updatingCells, onCellUpdate, onOpenLogs, loggedPunches, canWrite }: any) {
 ...
 <tr className="hover:bg-slate-50 relative group bg-white h-10">
      <td className="sticky left-0 z-20 bg-white group-hover:bg-slate-50 border border-slate-400/50 px-0.5 py-1 text-center text-[10px] md:text-[11px] font-bold text-slate-800 w-[24px] md:w-[40px]">{idx + 1}</td>
      <td className="sticky left-[24px] md:left-[40px] z-20 bg-white group-hover:bg-slate-50 border border-slate-400/50 px-1 md:px-2 py-1 text-[9px] md:text-[11px] font-black text-slate-900 truncate max-w-[90px] md:max-w-[180px] lg:max-w-[256px] w-[90px] md:w-[180px] lg:w-[256px]">{worker.name} {worker.last_name}</td>
      <td className="sticky left-[114px] md:left-[220px] lg:left-[296px] z-20 bg-white group-hover:bg-slate-50 border border-slate-400/50 px-1 md:px-2 py-1 text-[8px] md:text-[10px] font-bold text-slate-500 truncate max-w-[56px] md:max-w-[120px] lg:max-w-[144px] w-[56px] md:w-[120px] lg:max-w-[144px]">{worker.position || '-'}</td>
